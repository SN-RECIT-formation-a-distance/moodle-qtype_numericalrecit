...
class Utilities {
  ...
  breakpoint = 1224;
	...
	create = () => {
		this.mainDoc = document.getElementById('main-doc');
...
class Nav {
  ...
  classes = {
	...
	create = () => {
		...
		this.sideBtn = document.getElementById('nav-side-btn');
	}
  ...
  handleNav = (addClass, removeClass, open) => {  
    ...
    state.navOpen = open;
    cookies.set('navOpen', state.navOpen)
    this.closeBtnToggle();
  ...
  sideToggle = () => {
    ...
    cookies.set('navSide', state.navSide);
...
class Headings {
  ...
  create = () => {
	this.all = document.querySelectorAll('h2, h3, h4');
...
class HashLinks {
  ...
  scrollTo = () => {
    ...
    for (let i = 0; i < headings.list.length; i++) {
      ...
      if (tag == href){
        headings.all[i].scrollIntoView();
        document.body.scrollTop = headings.all[i].offsetTop;
        break;
      }
    }
  };
...
class TableOfContents {
  ...
  linkClick = e => {
    ...
    allowScroll();
    isSmallScreen() ? setTimeout(() => loc.scrollIntoView(), 500) : loc.scrollIntoView();
  };
  ...
  highlight = () => {
    ...
    for (let i = 0; i < num; i++) {
      ...
      const nearBot = false; //document.documentElement.scrollHeight - (window.innerHeight + window.pageYOffset) < 0;
  ...
  create = () => {
	this.element = document.getElementById('toc');
...
class ReadingBar {
  ...
  create = () => {
	this.container = document.getElementById('reading-bar');
  ...
  setProgress = () => {
    ...
    const docHeight       = Math.max(document.body.clientHeight - windowHeight, windowHeight, document.body.scrollHeight - windowHeight);
...
class ScrollToTop {
  ...
  create = () => {
	this.btn = document.getElementById('scroll-btn');
  }
...
class FontSize {
  ...
  _max = 3;
	...
	create = () => {
		this.increaseBtn = document.getElementById('zoom-in');
		this.decreaseBtn = document.getElementById('zoom-out');
	}
  ...
  change = (thisBtn, thatBtn, increment) => {
    ...
    const style = window
      .getComputedStyle(utilities.mainDoc, null)
    ...
    cookies.set('fontInt', state.fontInt);

    utilities.mainDoc.style.fontSize = `${currentSize}px`;
...
window.addEventListener('load', () => {
  ...
  setTimeout(() => hashLinks.scrollTo(), 1000);
  readingBar.setProgress();
});
